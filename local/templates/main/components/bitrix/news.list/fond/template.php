	...
	<div class="news-item clearfix" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
		<div class="col-xs-3 left-col">
			<?if($arParams["DISPLAY_PICTURE"]!="N" && is_array($arItem["PREVIEW_PICTURE"])):?>
				<img class="news-item__img" src="<?=$arItem["PREVIEW_PICTURE"]["SRC"]?>" alt="<?=$arItem["PREVIEW_PICTURE"]["ALT"]?>" title="<?=$arItem["PREVIEW_PICTURE"]["TITLE"]?>"/>
			<?endif?>
		</div>
		<div class="col-xs-7 right-col">
			<p class="news-item__title"><?=$arItem["NAME"]?> <span class="news-item__date"><?=$arItem["DISPLAY_ACTIVE_FROM"]?></span></p>
